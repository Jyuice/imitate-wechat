...
export default class CommentArea extends Component {
  ...
  renderFunc = (responseId, userId, index) => {
    if(this.state.isShowFunc[index]) {
      if(userId === localStorage.getItem("userId")*1){
        return (
          <Fragment>
            <div className="func">
              <div className="item">复制</div>
              <div className="item" onClick={() => {this.delComment(responseId)}}>删除</div>
            </div>
    ...
    if(item.type*1) { // 0是评论，1是回复
      return (
        <Fragment>
          ...
          <span className="name">{item.commentor}</span>
        </Fragment>
      )
    }else{
      return
      ...
      return (
        <div className="comment-area">
          {this.state.list.map((i,index) => (
            <div className="box"
            onClick={() => this.toReply([i.moment_id,i.response_id,i.commentor,i.responser_id])}
            onTouchStart={() => this.touchStart(index)}
            onTouchMove={this.touchMove} 
            onTouchEnd={this.touchEnd}
            key={i.response_id} id={i.response_id}>
              <span className="name">{i.commentor}</span>
              {this.renderReply(i)}
              ：
