...
import { getMomentLikers } from "../../api/moment";
import { io } from "socket.io-client";
const socket = io.connect('ws://106.52.45.38:6050/moment')

export default class ThumbUpArea extends Component {
  state = {
    thumbUpList: [],
  };

  componentDidMount() {
    this.getMomentLikers();
    this.props.getThumbUpArea(this);
    socket.emit("online",localStorage.getItem("userId"))
  }

  doLike = isLike => {
    const { thumbUpList } = this.state
    ...
    if (isLike) {
      ...
      let newList = thumbUpList
        .slice(0, index)
        .concat(thumbUpList.slice(index + 1, thumbUpList.length));
      this.setState({
        thumbUpList: newList,
      });
    }
  ...
  sendRealTimeUpdate = () => {
    socket.emit("online",localStorage.getItem("userId"))
    socket.emit("do-like-moment", this.props.momentId,data => {
      console.log(data)
    })
  }
  ...
  doRealTimeUpdate = () => {
    socket.emit("online",localStorage.getItem("userId"))
    socket.on("renew-thumbup-area", (momentId,fn) => {
      fn()
      this.getMomentLikers(momentId)
    })
  }

  getMomentLikers = (momentId=this.props.momentId) => {
    const params = {
      userId: localStorage.getItem("userId"),
      momentId: momentId,
    };
    getMomentLikers(params).then((res) => {
      let list = [];
      for (let i of res) {
        list.push(i.nickname);
        if (i.nickname === localStorage.getItem("nickname")) {
          this.props.getIsLike();
        }
      }
      this.setState({
        thumbUpList: list,
  ...
  renderComma = (i) => {
    const { thumbUpList } = this.state;
    if (thumbUpList.lastIndexOf(i) === thumbUpList.length - 1) return;
    else return <span>，</span>;
  };

  render() {
    const { thumbUpList } = this.state;
    if (thumbUpList.length) {
      return (
        <div className="thumb-up-area">
          <svg
            t="1627834145100"
            className="icon"
