...
export default class EachMoment extends Component {
  ...
  fuzzUpTime = time => {
    ...
    const day_now = Number(date.getDay())
    ...
    if(month_now - month > 0) jatlag = month_now - month + "月前"
  ...
  render() {
    ...
    const { nickname,avatar,moment_id,content,time } = this.props.each
    this.fuzzUpTime(time)
    return (
