...
export default class EachMoment extends Component {
  ...
  fuzzUpTime = () => {
    const { time } = this.props.each
    const year = Number(time.slice(0,4))
    const month = Number(time.slice(5,2))
    const day = Number(time.slice(9,2))
    const hour = Number(time.slice(12,2))
    const min = Number(time.slice(15,2))
