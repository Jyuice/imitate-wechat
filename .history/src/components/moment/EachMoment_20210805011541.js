...
export default class EachMoment extends Component {
  ...
  doLikeMoment = () => {
    const { isLike, thumbUpList, name } = this.state
    if(isLike === 0) {
        this.setState({
            isLike: 1,
            likeContent: "取消"
        ...
        // 删除自己的名字
        const index = thumbUpList.indexOf(name)
        let newList = thumbUpList.slice(0,index).concat(thumbUpList.slice(index+1,thumbUpList.length))
        this.setState({
            thumbUpList: newList
        })
    }
  ...
  renderDelBtn = (author_id, moment_id) => {
    if(localStorage.getItem("userId") === author_id) {
      return (
        <span href="#" onClick={this.delMoment(author_id, moment_id)}>删除</span>
      )
    }else return
  }

  delMoment = (author_id, moment_id) => {
    const data = {
      userId: author_id,
      momentId: moment_id
    }
  ...
  fuzzUpTime = time => {
    ...
    if(hour_now - hour > 0) jatlag = hour_now - hour + "小时前"
    if(day_now - day > 0) jatlag = day_now - day + "天前"
    if(month_now - month > 0) {
      if(month_now - month === 1 && day_now - day < 30) {
        jatlag = day_now + (bigMonth.indexOf(month) > -1 ? 31 : 30) - day + "天前"
      }
  ...
  render() {
    ...
    // const { nickname,avatar,moment_id,content,time,type,images } = this.props.each
    const { author_id, nickname,avatar,moment_id,content,time } = this.props.each
    return (
      <div className="each-moment" id={moment_id}>
        ...
        <div className="left">
          ...
          <ThumbUpArea momentId={moment_id} thumbUpList={this.state.thumbUpList} />
          <CommentArea momentId={moment_id} />
        </div>
      </div>
    );
  }
