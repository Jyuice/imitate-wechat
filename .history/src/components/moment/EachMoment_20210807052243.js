...
export default class EachMoment extends Component {
  ...
  getCommentArea = child => {
    this.$commentArea = child
  }
  ...
  toComment = e => {
    this.props.showInput(e.target.id,0) // 把moment_id传过去
    this.setState({
      isShowFunc : false
    })
  }
  ...
  updateComment = () => {
    const { isUpdate, momentId } = this.props.updateComment
    // console.log(isUpdate,momentId)
    if(isUpdate) {
      this.$commentArea.getComments(momentId)
    }
  }
    ...
    return (
      <div className="each-moment" id={moment_id}>
        ...
        <div className="left">
          ...
          <CommentArea
            momentId={moment_id}
            getCommentArea={this.getCommentArea}
            updateComment={this.props.updateComment}
            toReply={this.props.showInput} // 把父级的showInput函数传给子级，以供回复使用
          />
        </div>
      </div>
    );
