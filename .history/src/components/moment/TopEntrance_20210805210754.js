...
import { changeMomentBg, postImages } from '../../api/moment';
...
class TopEntrance extends Component {
    ...
    state = {
        ...
        moment_bg: localStorage.getItem("moment_bg") || ""
    }
    ...
    getUserInfo = () => {
        ...
        getUserInfo(localStorage.getItem("userId")).then(res => {
            if(res.nickname !== localStorage.getItem("nickname") ||
                res.avatar !== localStorage.getItem("avatar") ||
                res.moment_bg !== localStorage.getItem("moment_bg")){
                this.setState({
                    nickname: res.nickname,
                    avatar: res.avatar,
                    moment_bg: res.moment_bg
                })
                localStorage.setItem("nickname",res.nickname)
                localStorage.setItem("avatar",res.avatar)
                localStorage.setItem("moment_bg",res.moment_bg)
            }
        })
    }
    ...
    toPostMoment = () => {
        ...
        postImages(formData).then(res => {
            let filesArr = res.files.split(";")
            this.props.history.push(`/discover/moment/post`,{type: 1, filesArr: filesArr})
        })
    ...
    touchStart = () => {
        this.timeOutEvent = setTimeout(() => {
            this.props.history.push(`/discover/moment/post`,{type: 0})
        }, 800);
    }
    ...
    render() {
        const {nickname, avatar, moment_bg} = this.state
        return (
            <div className="top">
                <div className="icon">
                    <Icon className="left" type="left" onClick={this.goback}></Icon>
                    <form action="" encType="multipart/form-data" style={{display: "none"}}>
                        <input ref={this.fileRef} onChange={this.toPostMoment} maxLength={9}
                        type="file" name="image" id="" style={{display: "none"}} multiple="multiple" />
                    </form>
                ...
                <form action="" encType="multipart/form-data" style={{display: "none"}}>
                    <input ref={this.bgRef} type="file" onChange={this.doChangeBackground}
                    name="background" id="" accept=".jpg,.png,.jpeg,.jfif"/>
                </form>
