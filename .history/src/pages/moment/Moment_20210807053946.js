...
export default class Moment extends Component {
  ...
  showInput = (id, type) => {
    this.inputType = type
    // 收到要评论的moment_id（type为0）或要回复的评论所属的moment_id（type为1）
    if (!(type * 1)) { // 给pyq评论
      ...
      this.setState({
        value: id * 1 === this.moment_commentToReplyId * 1 ? this.state.value : "",
        isShowInput:
          id * 1 === this.moment_commentToReplyId * 1
            ? !this.state.isShowInput
            : true,
      });
      this.moment_commentToReplyId = id * 1;
    }
  };
        ...
        <Fragment>
          <div className="comment-input">
            <Input
              ...
              name=""
              id=""
              placeholder={this.inputType*1 ? "回复" : "评论"}
            ></Input>
            <svg
              t="1627607851197"
              className="icon"
  ...
  send = () => {
    if(!this.inputType*1) { // 是评论朋友圈
      ...
      const data = {
        userId: localStorage.getItem("userId"),
        commentId: this.moment_commentToReplyId,
