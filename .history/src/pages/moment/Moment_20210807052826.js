...
export default class Moment extends Component {
  ...
  showInput = (id, type) => {
    this.inputType = type
    // 收到要评论的moment_id（type为0）或comment_id（type为1）
    if (!(type * 1)) { // 给pyq评论
      ...
      this.setState({
        value: id * 1 === this.commentToReplyId * 1 ? this.state.value : "",
        isShowInput:
          id * 1 === this.commentToReplyId * 1
            ? !this.state.isShowInput
            : true,
      });
      this.commentToReplyId = id * 1;
    }
  };
  ...
  send = () => {
    if(!this.inputType*1) { // 是评论朋友圈
      ...
      const data = {
        userId: localStorage.getItem("userId"),
        commentId: this.commentToReplyId,
