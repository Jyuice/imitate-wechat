...
import { getUserInfo, isFriend, sendApplication, deleteFriend } from "../../api/user";
...
export default class BusinessCard extends Component {
  ...
  todelFriend = () => {
    deleteFriend({
      ...
      friendId: this.props.location.state.userId
    }).then(res => {
      if(res.status === 200) {
        this.props.history.go(-1)
      }
  ...
  render() {
    return (
      <div>
        ...
        >
          <Button style={{color: 'red'}} onClick={this.todelFriend}>删除好友</Button>
        </Popup>
      </div>
    );
  }
