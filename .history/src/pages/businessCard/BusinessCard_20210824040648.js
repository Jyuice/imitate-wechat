...
const socket = io.connect('ws://106.52.45.38:6050/home/chat/business')
...
export default class BusinessCard extends Component {
  ...
  renderFunc = () => {
    if (
      this.state.isFriend ||
      this.props.location.state.userId === localStorage.getItem("userId")*1
    ) {
      return (
        <Fragment>
          {lists.map((i) => (
      ...
      return (
        <Fragment>
          ...
          <Button style={btnStyle} onClick={this.doAdd}>添加到通讯录</Button>
        </Fragment>
      );
    }
  };
  ...
  doAdd = () => {
    alert("", "是否确定加好友？", [
      { text: "取消", onPress: () => console.log("cancel"), style: "default" },
      {
        ...
        onPress: () => {
          Toast.success("已发送")
          // socket.emit("send-add",this.props.location.state.userId+"")
        },
      },
    ]);
  }
