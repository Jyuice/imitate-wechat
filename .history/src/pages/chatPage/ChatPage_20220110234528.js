/**
 * 
   this.props.history.push("/home/chat",{id: id,name: name,avatar: avatar, manager_id: manager_id})
 * 
 */
...
import { Popup, Button } from 'antd-mobile-v5'
...
import { getGroupMsgs, postGroupMsg } from "../../api/group"
...
export default class ChatPage extends Component {
  ...
  componentDidMount() {
    this.isGroup = !this.props.location.state.avatar
  ...
  // 如果我是群主，则可以解散群聊
  renderDissolve = () => {
    const { manager_id } = this.props.location.state
    if(manager_id === myId) {
      return (
  ...
  render() {
    ...
    return (
      <div className="chat-page">
        ...
        >
          <Button style={{color: 'red'}} onClick={this.todelFriend}>退出群聊</Button>
          {this.renderDissolve()}
        </Popup>
      </div>
    );
