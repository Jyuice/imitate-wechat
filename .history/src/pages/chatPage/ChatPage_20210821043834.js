    ...
    return (
      <div className="chat-page">
        ...
        {this.state.msgList.map((i, index) => {
          const isLastMsg = this.state.msgList.length - 1 === index
          console.log(index===this.state.msgList.length-1,index)
          return (
            <Bubble ref={isLastMsg ? this.lastRef : null} key={i.id} leftAvatar={avatar} msg={i} />
          )
        })}
