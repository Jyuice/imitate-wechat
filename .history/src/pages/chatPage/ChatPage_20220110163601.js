...
import { getPersonalMsgs, postMsg } from "../../api/chat";
import io from 'socket.io-client'
import { getGroupMsgs } from "../../api/group"
...
getMsgs = () => {
      ...
      getPersonalMsgs(data).then(res => {
        if(!res.length) { // 没有记录
          ...
          for(let i of res) {
            const obj = {
              id: i.msg_id,
              userType: i.sender_id === localStorage.getItem("userId")*1 ? 0 : 1,
              content: i.msg,
              // 发送人的名称和头像
              avatar: i.avatar,
              nickname: i.nickname,
  ...
  sendMsg = () => {
    ...
    socket.emit("send-message", localStorage.getItem("userId"), this.props.location.state.id, this.state.value)
    // this.getSocket()
    // this.connSocket()
  }
