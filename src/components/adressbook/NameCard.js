...
class NameCard extends Component {
  ...
  toTarget = list => {
    ...
    if(list.target === "/home/chat") {
      this.props.history.push('/home/chat', {id: list.id,name: list.name,avatar: list.avatar, manager: list.manager_id})
    }else this.props.history.push(list.target)
  }

  renderCheck = () => {
    if(this.props.select) {
      return <Checkbox className="checkbox" onChange={this.checkList}></Checkbox>
    } else return
  }

  checkList = e => {
      this.props.getChildMsg(e.target.checked, this.props.list.id)
  }
