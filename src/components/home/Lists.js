...
class Lists extends Component {
  ...
  toChatPage = () => {
    const { user_id, friend_id, group_id,manager_id, avatar, name } = this.props.list
    this.props.history.push("/home/chat",{id: group_id ? group_id : (user_id === myId*1 ? friend_id : user_id),name: name,avatar: avatar, manager: manager_id})
  ...
  render() {
    const { avatar, group_avatar, name, msg, time} = this.props.list
    return (
      <div className="chat-lists-wrap" onClick={this.toChatPage}>
        <div className="left">
          <div className="avatar-container">
            <img src={baseUrl + (avatar ? avatar : group_avatar)} alt="" />
